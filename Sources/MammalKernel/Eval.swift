...
public func eval0<T>(_ node: Expr<T>, env: Environment<T>) throws -> Value<T> {
    switch node {
    ...
    case .App(let fn, let args):
        return try eval0(fn, env: env).withFn { (arity, f) in
            let argVs = try args.map { try eval0($0, env: env) }
    ...
    case .Quote(let expand):
        let splice = try expand { expr in
            try eval0(expr, env: env)
        }
        return try eval0(splice, env: env)
